...
def list_documents(
    ...
    rows, total = KnowledgeService(db).page_documents(kb_id, pageNo, pageSize, keyword)
    items = [
        {
            "id": row.id,
            "kbId": row.kb_id,
...
def chunk_logs(doc_id: str, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    ...
    return success([{"id": row.id, "status": row.status, "message": row.message, "chunkCount": row.chunk_count, "createdAt": row.created_at.isoformat()} for row in rows])

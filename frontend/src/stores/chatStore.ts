...
export const useChatStore = defineStore('chat', () => {
  ...
  async function sendMessage(content: string, attachments?: ChatAttachment[]) {
    ...
    messages.value.push({ role: 'assistant', content: '', agentEvents: [] })
        ...
        (event: any) => {
          if (event.type !== 'token') {
            ...
            if (messages.value[assistantIndex]?.agentEvents) {
              messages.value[assistantIndex].agentEvents!.push(event as StreamEvent)
            }
          }
          ...
          if (event.type === 'final_answer') {
            ...
            if (messages.value[assistantIndex]) {
              ...
              messages.value[assistantIndex].agentEvents = messages.value[assistantIndex].agentEvents || []
            }
          }
          if (event.type === 'token') {
            if (messages.value[assistantIndex]) {
